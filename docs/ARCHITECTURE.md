...
## Private photography and exam extension — September 16

Migration `20260916174953_grading_photos_and_northside_exam.sql` follows unchanged 001–009 and adds five tenant/card-scoped photo/draft/revision tables with runtime RLS, compound identity keys and immutable publication/photo snapshot triggers. `lib/server/exam.ts` owns role/ownership checks, versioned drafts, stable upload attempts, server-confirmed completion and explicit publication. `exam-storage.ts` fully decodes/orients bounded input and uses no-overwrite private objects with stored-byte readback/hash confirmation. Failure preserves the received card and pending attempt. Publication rechecks actual storage before inserting an immutable signed revision. Import reversal now rejects photo/draft activity.

`exam-http.ts` is shared by the authenticated private Node route and the strictly guarded local preview route. Customers get only published revisions and their allowed photos; staff-only internal notes/reasons live separately. Image downloads and reports authorize each request and use no-store headers. Reports use separate authenticated image URLs instead of base64 expansion. No image goes through the public image optimizer or PWA cache. `exam-report.js` waits for image loading before printing and preserves the session privacy lock.

`components/northside-exam.tsx` provides native camera/file inputs, previews, upload progress/retry, zoom, initially blank manual score fields, draft saving and explicit examiner signoff. Pages are under `/staff/grading/exam/[id]` and `/my-cards/grading/exam/[id]`. Fixture objects persist outside public assets in ignored `work/grading-photos/`, alongside the existing ignored database. See NORTHSIDE-EXAM.md for recovery, formats, production validation and external limitations.

## Customer grading and approval enforcement — September 16

`grading-portal.ts` separates explicit customer projections from the staff workspace, with owner-scoped cards, photos, receipts, immutable quote/exam revisions and decision history. `grading-portal-http.ts` shares verified-session, same-origin and no-store boundaries between private APIs and guarded local fixtures. `grading-receipt.ts` produces escaped printable receipts; owned CSV excludes staff/batch fields. The app `/grading` entry and website both lead to this one portal; actual Square navigation is deferred, not represented by a duplicate website database or identity system.

Migration `20260916185139_customer_grading_quotes_and_approvals.sql` adds immutable quotes, authenticated decision requests and selected-card evidence with composite tenant/card keys and RLS. The private, restricted `ns.grading_approve` function locks the exact cards, validates ownership/state/current exam and quote, records actor/time and applies the selected statuses atomically. Stable request identities support response-loss recovery. Current approval is computed from immutable latest decisions and current evidence; it is never a client-supplied flag. Quote/exam/dispatch writers coordinate on the same card locks. Triggers reject dispatch without current approval and a matching confirmed batch provider/service. The prior consent function rejects new use.

Migration `20260916192111_grading_batch_service_setup.sql` allows an older batch's missing service to be configured before first dispatch and then freezes provider/service and membership using immutable dispatch history. Current confirmed non-paper photos are now owner-visible before publication, while draft assessments and paper/internal evidence remain private. No service prices are seeded. See [CUSTOMER-GRADING.md](CUSTOMER-GRADING.md) for contracts, local tests and unverified hosted concurrency.

## Staff grading custody extension

`lib/server/grading-fulfillment.ts` and `grading-status-import.ts` back the staff-only operations routes/workspace. The additive custody migration preserves staged scans, immutable dispatch and ownership/approval/quote/exam snapshots, external outcome revisions, verified per-card pickups, durable exact-payload retries, reviewed import identities and post-dispatch withdrawal reviews. Server roles/RLS and explicit portal projections retain owner privacy. A tenant custody lock precedes batch/card locks; dispatch locks/rechecks current customer approval. Later physical milestones use the preserved shipment evidence. Original and returned photos share private verified storage but remain separate snapshots. Notification rows/jobs are transactional and deduplicated. See [staff workflow and security](STAFF-GRADING-OPERATIONS.md); no live grader feed, payment or mail send is introduced.

## Final grading concurrency verification

September 16 acceptance found a real PostgreSQL joined-row recheck issue: `gradingCard(..., true)` now locks only the scoped grading-card identity, then reads the joined state/metadata with a fresh statement. This avoids false 404s after a waiting concurrent status change. The standalone `scripts/grading-concurrency.ts` proves six overlapping local transactions with real PostgreSQL locks, supplementing PGlite tests; hosted identity/pooling/Storage remain unverified. See [consolidated acceptance](grading-intake.md).
